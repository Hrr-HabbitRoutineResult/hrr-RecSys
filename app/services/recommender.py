...
def recommend(query: str, items: List[ChallengeItemIn], top_k: int) -> RecommendResponse:
    ...
    # 2) 아이템 임베딩 행렬
    embs = np.vstack([np.asarray(it.embedding, dtype=np.float32) for it in items])  # (N, D)

    # 3) 기본 점수 (코사인) + 재랭킹 보너스
    base_scores = _base_similarity(q_vec, embs)             # (N,)
    boosted = _apply_boosts(base_scores, items, qtext)      # (N,)
